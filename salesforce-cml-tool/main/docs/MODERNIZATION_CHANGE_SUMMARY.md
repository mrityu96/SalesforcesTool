...
**Release:** 1.0.0  
**Validation status:** 141 Python tests and 23 Chromium Playwright tests passing,
plus focused Firefox and WebKit UX/security checks
...
file-by-file diff for the modernization. When the project is added, commits
should include the application, policies, workflows, and tracked development
tests/configuration.

The root `.gitignore` excludes generated dependencies, browser
binaries/results/caches, release output, and potentially sensitive
runtime/recovery artifacts—not the complete development workspace.
...
The operator UI now has four views. Best-practice checking is inline in Fetch
& Deploy with an explicit hide-and-clear control, and deployment readiness is
limited to the exact target version's Active/Inactive/write-blocked status.
The former Context Definition discovery/evidence runtime path and its extra
describes/queries were removed; Context Definition prerequisites remain
...
tracked `development/`. Dependencies, browser binaries/results/caches, release
output, and runtime artifacts remain ignored. Runtime and recovery output
defaults to `development/runtime/`; `CML_RUNTIME_ROOT` can override it.

### Versioned release surface

- Stable initial version `1.0.0` reflects the completed public operator
  contract; future incompatible changes require a major release.
- `CHANGELOG.md`, `SECURITY.md`, `COMPATIBILITY.md`, `CONTRIBUTING.md`,
  root `LICENSE`, `VERSION`, and corrected `THIRD_PARTY_NOTICES.md` define the
  release and support boundary.
- CI verifies Python 3.9/3.13, browser behavior, the locked offline editor
  bundle, security headers, private-artifact exclusions, and deterministic
  release archives.
- Version tags invoke a least-privilege release workflow only after reusable CI
  succeeds; operator `.tar.gz` and `.zip` archives are allowlisted, inspected,
...
- `templates/index.html`, `assets/`, and `app/cml_tool_page.py`
  - Separate the HTML shell, cohesive local CSS/JavaScript, offline CodeMirror
    bundle, and thin template loader.
  - Provide reusable multi-instance CodeMirror editing for primary and target
    drafts, line numbers, semantic overlays, merge rail, target draft copy,
    comments, and cancellation UI.
  - Keep best-practice results beside the primary editor, where they can be
    hidden together with their inline diagnostics and rerun on demand.
  - Provide one accessible typed-confirmation dialog across all write/recovery
    paths, exact-selection session draft recovery, keyboard shortcut help,
    live/busy status semantics, reduced motion, and responsive reflow.
- `app/cml_analysis.py`
...
- Cohesive `test_cml_analysis.py`, `test_cml_http.py`,
  `test_cml_lifecycle.py`, and `test_cml_constraints.py` modules replace the
  former large `test_cml_tool.py` without duplicate discovery.
- `development/tests/test_release_artifacts.py`
  - Deterministic archive, exact-content, exclusion, and checksum coverage.
- `development/tests/browser/cml_tool.spec.js`
  ...
  - Covers Fetch-button placement, scoped runtime-status labels, comments and
    ...
    comparison, server-aware cancellation, compare CodeMirror, keyboard-only
    confirmation, session draft recovery, and native-dialog exclusion.
...
- `salesforce-cml-tool/main/docs/CML_TOOL_COMPLETE_GUIDE.md`
  - Documents the modular architecture, exact-version safety contract, API
    ...
    maintenance.
- `salesforce-cml-tool/main/docs/screenshots/*-latest.png`
  - Current synthetic walkthroughs for fetch/deploy, semantic comparison,
...
- Maintains in-process and cross-process deployment locks plus operation
...
- The read-only readiness endpoint resolves only the exact target and reports
  its current Active/Inactive/write-blocked eligibility; it does not inspect
  CML references or discover Context Definition metadata.
...
- Separate Best Practices navigation and dedicated view
- Context Definition reference/path discovery, evidence payloads, and related
  readiness describes/queries

### Files actually deleted

No file deletion can be confirmed from Git because the project directory is
untracked. Ignored generated/private development paths are not production
deletion evidence. In the current workspace:
...
- **141 of 141 Python tests passed**
- **23 of 23 Playwright browser tests passed using the pinned Chromium test binary.**
- Focused offline/CSP and 200% zoom tests also pass under Firefox and WebKit.
...
- Central retention/legal-hold and additional redaction controls
- Configurable Salesforce API version with capability detection
- Exportable dependency manifests for blocked prerequisites

The full read-only contract harness passed against the named `migDev` sandbox:
11 capabilities passed and only the intentionally unavailable write scenario
was skipped. No Salesforce write, save/restore, activation, or runtime scenario
was executed. Runtime reliability therefore remains the only score below 8.5
until a separately approved, numbered non-production write-validation run is
completed; read-only evidence must not be presented as write-path acceptance.
