...
21. [Acceptance scorecard](#21-acceptance-scorecard)
...
- Tokenizer, tolerant parser, and semantic comparison:
  `salesforce-cml-tool/main/app/cml_analysis.py`
- Browser shell and static guide: `salesforce-cml-tool/main/templates/index.html`
- Browser styles, application modules, editor adapter, and offline CodeMirror:
  `salesforce-cml-tool/main/assets/`
- Template loader: `salesforce-cml-tool/main/app/cml_tool_page.py`
...
tests and guarded adapters, loads the page through `app/cml_tool_page.py`, and
starts the local server. HTML, CSS, and JavaScript are maintained as local
modules; only the committed CodeMirror bundle has a reproducible development
build step. The optional shell/Python utility launchers delegate to
...
- **Portable key:** A user-selected field whose value is expected to identify corresponding records across orgs. Suggestions are discovered from live object descriptions in the selected orgs; `Global_Key__c` is not assumed to exist.
...
No key is preselected. The tool discovers filterable business-key candidates from live descriptions of `Product2`, `ProductClassification`, and `ProductRelatedComponent` in the selected orgs. A suggestion appears only for the same supported object type in every selected org. The tool also accepts a plain Salesforce field API name of up to 80 characters. `Name`, `ProductCode`, or another external/custom identifier can be used only where the target object exposes it and the deployment process establishes that it is populated, stable, and unique.
...
1. **Composition root:** `app/cml_tool.py` loads sibling modules, exposes the
   ...
   `127.0.0.1`.
2. **Browser UI:** HTML is stored in `templates/index.html`; cohesive CSS and
   JavaScript modules plus the offline CodeMirror bundle are under `assets/`.
   `app/cml_tool_page.py` is only the template loader. CodeMirror is reproduced
   from the locked development graph; runtime needs no Node installation.
...
The application uses one full-width shell with a compact header and a single
floating top navigation island. The header retains Salesforce/CML branding,
creator credit, Donate/About, local-runtime status, build identity, and the
light/dark theme control. There is no secondary sidebar or duplicate
navigation. Source and target selection remain visible as paired workspaces and
stack intentionally on narrow screens.
...
- source-to-target draft merge arrows;
- a second, independently managed offline CodeMirror target-draft editor for
  arbitrary line and comment changes; and
...
CML** always copies that complete current target state. **Edit target** switches
the target pane to the same line-numbered local CodeMirror experience as the
primary editor. Saving direct edits reruns the line diff
and protected local semantic comparison; cancelling discards unsaved typing.
Merge and manual edits share the same target draft, reset baseline, and
**Review & Deploy target draft** handoff. A draft changes no Salesforce data
until it is loaded into the editor and sent through the normal guarded deploy
workflow.

The primary editor marks unsaved changes and saves a debounced recovery copy to
`sessionStorage`, keyed by exact source org, model, and version. The copy remains
only in the current browser tab/session; it is never sent to cloud storage or
telemetry. Returning to the same selection presents explicit **Restore draft**
and **Discard** choices. A successful deployment, a fresh fetch, or an explicit
discard clears the scoped copy. Closing or reloading with unsaved changes also
triggers the browser's before-unload protection.

All write and recovery operations use one in-app typed-confirmation dialog.
It summarizes the exact action and target, requires an exact alias match before
enabling submit, supports Escape, traps keyboard focus while open, and returns
focus to the initiating control. Native `prompt()` and `confirm()` are not used.
The header shortcut surface documents Cmd/Ctrl+S, Cmd/Ctrl+/, editor search, and
Escape behavior. Status regions announce updates and expose busy state; visible
focus, reduced-motion behavior, and responsive reflow support keyboard and
zoomed use.
...
Findings include line navigation, explanation, and generated before/after
guidance where available. Best-practice checking exists only beside the editor
in Fetch & Deploy. **Hide best practices** removes the report and clears inline
editor diagnostics; running the check again produces a fresh report. The
linter runs in the browser and does not send editor text to an analysis
...
deployment process. The CML Tool does not deploy Context Definition metadata,
and CML or ESCO deployment alone does not prove that those attributes will
resolve at runtime.
...
- must be selected explicitly; no org-specific custom field is assumed;
- is suggested from live source/target object descriptions;
...
### 10.7 Process-safe deployment locks

Write and recovery operations are serialized by `(target org, model)` within
one Python process and by a non-blocking advisory lock file under the runtime
`locks/` directory. A second local process using the same runtime root is
rejected immediately. This is not a distributed lock: different hosts or
different `CML_RUNTIME_ROOT` values do not coordinate.
...
Recovery artifacts are stored on the launching computer. There is no remote
durable store, encryption layer beyond operating-system controls, or central
audit collector. Local age-based retention exists, but it is not a legal-hold
or enterprise records-management system.
...
From `salesforce-cml-tool/`, enter the development workspace before running
tests:
...
Then refresh the CML Tool. Do not copy another user's credential files.
...
Recognized JSON recovery artifacts are retained for 90 days by default and
pruned during artifact operations. Set `CML_ARTIFACT_RETENTION_DAYS` to a
positive day count, or `0` to disable automatic pruning. Pruning reads each JSON
kind first; unrelated, malformed, and non-JSON files are not deleted. Define
the organizational policy for:
...
- `app/cml_tool.py` is the approximately 846-line composition root. It loads
  ...
  retains compatibility wrappers and patch points, owns process-safe deployment locks and
...
- `app/cml_analysis.py` owns tokenization, tolerant parsing, source ranges, and
  semantic comparison.
- `app/cml_tool_page.py` loads `templates/index.html`.
- `templates/index.html` and `assets/` own the browser shell, modular styles and
  scripts, offline CodeMirror adapter, windowed Myers comparison, semantic
  overlays/draft merges, target copy, and browser cancellation behavior.
...
The build hash covers `VERSION`, the composition root, all seven sibling
application modules, templates, and assets; changing any causes a changed-build
...
|---|---|---|---:|
| GET | `/` | Embedded UI | No |
| GET | `/api/ping` | App/version/build identity and local request token | No |
...
Scores are evidence-based snapshots for release 1.0.0, not guarantees. A score
is at least 8.5 only where the corresponding acceptance evidence exists.

- **Problem fit — 9.1/10.** Exact-version CML, semantic review, explicit
  operator prerequisite guidance, ESCO portability, and recovery directly
  cover the operator workflow.
- **Feature completeness — 8.8/10.** Fetch, editable compare/merge, lint,
  exact-target status, guarded deployment, selectable recovery, and static
  guidance are implemented. Context Definition validation and activation
  remain deliberately external.
- **Deployment safety and recovery — 9.0/10.** Exact ownership/status checks,
  typed confirmation, pre-write backup, SHA-256/read-after-write verification,
  rollback, authoritative delete archives, partial-result reporting, and
  process-safe locks have automated acceptance coverage.
- **Security and privacy — 8.8/10.** Loopback binding, Host/Origin/CSRF/body/CSP
  controls, generic 500 responses, redacted structured diagnostics, private
  atomic artifacts, path checks, DML allowlists, leak checks, and security
  guidance are present.
- **Documentation — 8.8/10.** README, complete operator/maintainer guide,
  in-product static guide, compatibility/support policy, security policy,
  contribution guide, changelog, notices, and score boundaries are aligned.
- **UX engineering acceptance — 9.5/10.** Both editing surfaces use reusable,
  offline CodeMirror instances; one accessible typed-confirmation dialog guards
  all destructive/write paths; scoped session-only draft recovery, dirty and
  before-unload protection, shortcut help, live/busy states, visible focus,
  reduced motion, responsive reflow, inline best-practice diagnostics, and
  a clear-and-rerun control plus automated four-view keyboard coverage are
  implemented. The complete workflow runs in
  Chromium, while offline/CSP and 200% zoom acceptance also run in Firefox and
  WebKit. This engineering score is not a measured usability-study score;
  representative keyboard and screen-reader operator sessions remain required
  before making claims about human task-completion or satisfaction.
- **Runtime reliability — 8.4/10 (acceptance blocker).** Local budgets,
  cancellation, structured Salesforce errors, retry/verification behavior,
  retention, and cross-process locking are covered by synthetic tests. A full
  read-only contract run against the named `migDev` sandbox passed 11
  capabilities with only the intentionally unavailable write scenario skipped.
...
- **Testing — 9.1/10.** 141 Python tests and 23 Playwright tests cover the
  ...
  without live org calls.
- **Maintainability — 8.6/10.** Backend services, frontend template/assets, and
  the former monolithic Python tests are split by responsibility with stable
  adapters. `main/assets/js/app.js` remains a sizeable workflow coordinator and
  is the primary future decomposition opportunity.
- **Release readiness — 8.7/10.** SemVer metadata, policies, accurate
  third-party notices, reusable CI, least-privilege tag release automation,
  deterministic dual-format operator archives, exact content verification, and
  SHA-256 checksums are implemented and dry-run tested.
