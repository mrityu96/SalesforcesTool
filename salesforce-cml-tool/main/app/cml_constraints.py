...
def _impl_list_key_fields(source_org, target_org=None):
    """Discover usable portable-key candidates from live object describes."""
    orgs = [org for org in (source_org, target_org) if org]
    if not orgs:
        return {"ok": False, "fields": [], "log": "Select a source org first."}
    ...
    for org in orgs:
        ...
        if err:
            return {"ok": False, "fields": [], "log": err}
        described[org] = {}
        for sobject in _get("REF_TYPES"):
            url = (
                f"{instance}/services/data/{_get('API_VERSION')}/sobjects/"
                f"{sobject}/describe")
            payload, request_err = _rest("GET", url, token)
            if request_err and _get("_is_auth_error")(request_err):
                token, instance, refresh_err = _org_creds(org, refresh=True)
                if refresh_err:
                    return {"ok": False, "fields": [], "log": refresh_err}
                url = (
                    f"{instance}/services/data/{_get('API_VERSION')}/sobjects/"
                    f"{sobject}/describe")
                payload, request_err = _rest("GET", url, token)
            if request_err:
                return {
                    ...
                    "fields": [],
                    "log": (
                        f"Could not describe {sobject} in {org}: "
                        f"{request_err}"),
                }
            fields = {}
            for field in (payload or {}).get("fields", []):
                name = field.get("name")
                if not name or not field.get("filterable"):
                    continue
                fields[name] = field
                metadata.setdefault(name, field)
            described[org][sobject] = fields
    ...
    known_business_fields = {"Name", "ProductCode", "StockKeepingUnit"}
    candidates = []
    for name, field in metadata.items():
        if name in excluded:
            continue
        if not (
                name in known_business_fields
                or name.endswith("__c")
                or field.get("externalId")
                or field.get("unique")):
            continue
        shared_types = [
            sobject for sobject in _get("REF_TYPES")
            if all(name in described[org][sobject] for org in orgs)]
        if not shared_types:
            continue
        candidates.append({
            "name": name,
            ...
            "objectTypes": shared_types,
            "allReferenceTypes": len(shared_types) == len(_get("REF_TYPES")),
            "warning": (
                "Name matching requires the same populated value in both orgs "
                "and duplicates are blocked."
                if name == "Name" else None),
        })
    candidates.sort(key=lambda item: (
        0 if item["externalId"] or item["unique"] else 1,
        0 if item["allReferenceTypes"] else 1,
        1 if item["name"] == "Name" else 0,
        item["name"].lower(),
    ))
    return {
        "ok": True,
        ...
        "fields": candidates,
        "objectTypes": list(_get("REF_TYPES")),
        "scopeNote": (
            "Fields are discovered from live Salesforce object descriptions. "
            "A field is listed only where it is filterable and exists on the "
            "same supported reference-object type in every selected org."),
    }
...
def _impl_export_constraints(org, model, version_id,
    ...
    if ownership_err:
        return {"ok": False, "log": (
            ...
            f"version:\n{ownership_err}")}
    if not str(key_field or "").strip():
        return {"ok": False, "log": (
            "Choose a foreign-key field discovered from the selected org "
            "before loading constraint data.")}
...
def _impl_compare_constraints(source_org, target_org, model, source_version_id,
    ...
    if not str(key_field or "").strip():
        return {"ok": False, "log": (
            "Choose a foreign-key field shared by the source and target orgs "
            "before comparing constraint data.")}
    kf = _valid_field(key_field)
    if not kf:
        return {"ok": False, "log": (
            f"\u201c{key_field}\u201d is not a valid field API name. Use a plain "
    ...
    for key in all_keys:
        ...
        for r in source_rows[paired:]:
            ...
            if "exact" in (r.get("dups") or []):
                ...
                row["deployStatus"] = "blocked"
                row["blockNote"] = (
                    f"No target {r['refType']} has {kf} matching "
                    f"'{r.get('gkey')}'. "
                    + (
                        "Name is supported, but its value must be populated, "
                        "match across both orgs, and resolve uniquely. "
                        if kf == "Name" else ""
                    )
                    + "This tool will not create or update catalog records; "
                    "deploy it separately, then compare again."
                )
