...
SEMANTIC_MAX_CHARS = _positive_env_int(
    "CML_SEMANTIC_MAX_CHARS", 4 * 1024 * 1024)
SEMANTIC_MAX_LINES = _positive_env_int(
    "CML_SEMANTIC_MAX_LINES", 100_000)
SEMANTIC_MAX_SECONDS = _positive_env_float(
    "CML_SEMANTIC_MAX_SECONDS", 5.0)
SEMANTIC_MAX_CONCURRENCY = _positive_env_int(
    "CML_SEMANTIC_MAX_CONCURRENCY", 2)
_SEMANTIC_SLOTS = threading.BoundedSemaphore(SEMANTIC_MAX_CONCURRENCY)


class _SemanticBudgetExceeded(Exception):
    """Raised when local semantic guidance exceeds its dedicated budget."""


class _SemanticBudget:
    def __init__(self, cancel_check=None):
        self.deadline = time.monotonic() + SEMANTIC_MAX_SECONDS
    ...
    def checkpoint(self):
        ...
        if time.monotonic() > self.deadline:
            raise _SemanticBudgetExceeded(
                f"Semantic analysis exceeded its {SEMANTIC_MAX_SECONDS:g}s "
                "time budget.")


def _empty_semantic_result(status, reason):
    return {
        "schemaVersion": "1.0",
        ...
        "stats": {
            key: 0 for key in (
                "UNCHANGED", "MOVED", "ADDED", "REMOVED", "MODIFIED",
                "AMBIGUOUS")
        },
        "sourceParseIssues": [],
        "targetParseIssues": [],
...
def _tokenize_cml(content, budget=None):
...
class _CmlParser:
    ...
    def __init__(self, content, tokens, diagnostics, budget=None):
...
def _semantic_index(content, budget=None):
    """Parse CML once and index named entities independently of line order."""
    tokens, diagnostics = _tokenize_cml(content, budget)
    parser = _CmlParser(content, tokens, diagnostics, budget)
...
def compare_cml_semantics(source_content, target_content, cancel_check=None):
    ...
    total_chars = len(source_content) + len(target_content)
    if total_chars > SEMANTIC_MAX_CHARS:
        return _empty_semantic_result(
            "unavailable",
            f"Semantic analysis skipped: {total_chars:,} characters exceed "
            f"the {SEMANTIC_MAX_CHARS:,}-character budget. Raw line "
            "comparison remains available.")
    total_lines = (
        source_content.count("\n") + target_content.count("\n") + 2)
    if total_lines > SEMANTIC_MAX_LINES:
        return _empty_semantic_result(
            "coarse",
            f"Semantic analysis skipped: {total_lines:,} combined lines "
            f"exceed the {SEMANTIC_MAX_LINES:,}-line budget. Raw line "
            "comparison remains available.")
    if not _SEMANTIC_SLOTS.acquire(blocking=False):
        return _empty_semantic_result(
            "unavailable",
            "Semantic analysis concurrency budget is busy. Raw line "
            "comparison remains available; retry the semantic summary.")
    budget = _SemanticBudget(cancel_check)
    try:
        budget.checkpoint()
        source_entities, source_issues, source_unknown = _semantic_index(
            source_content, budget)
        target_entities, target_issues, target_unknown = _semantic_index(
            target_content, budget)
    except _SemanticBudgetExceeded as exc:
        return _empty_semantic_result("bounded", str(exc))
    finally:
