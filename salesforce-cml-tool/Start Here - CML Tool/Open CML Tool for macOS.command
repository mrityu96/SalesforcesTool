#!/bin/bash
# ============================================================================
# Open CML Tool for macOS — double-click this file to open the Salesforce CML
...
# again any time to reopen the tool. To stop it, use
# "Stop CML Tool for macOS.command".
# ============================================================================
...
if echo "$RUNNING_PING" | grep -q "cml-tool"; then
  CUR_BUILD="$("$PY" app/cml_tool.py --print-build 2>/dev/null)"
...
for i in $(seq 1 40); do
  if curl -s "$PING" 2>/dev/null | grep -q "cml-tool"; then
    ...
    echo "To stop it later, double-click \"Stop CML Tool for macOS.command\"."
    exit 0
  fi
  sleep 0.5
done
